...
if (isset($_POST['submit'])) { // Vérification de la soumission du formulaire
    ...
    $userId = createUser($_POST['name'], $_POST['email'], $_POST['password'], $date); // Création de l'utilisateur
    if ($userId) { // Si l'utilisateur est créé
        $_SESSION['profil'] = ['name' => $_POST['name'], 'email' => $_POST['email'], 'idutilisateur' => $userId['id'], 'date' => $date, 'roles' => [$userId['roles']]];  // Enregistrement des informations de l'utilisateur dans la session
